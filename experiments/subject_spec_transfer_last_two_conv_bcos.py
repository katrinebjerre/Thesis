# experiments/subject_spec_transfer_last_two_conv_bcos.py
...
def main():
    ...
    TRANSFER_MODE = "last_two_conv"
    SETTINGS = {
        **get_base_settings(),
        "results_dir": "results/subject_spec_bcos",
        "results_file": f"subject_spec_transfer_{TRANSFER_MODE}_bcos.csv",
        "history_file": f"subject_spec_transfer_{TRANSFER_MODE}_bcos_history.csv",
    }
    ...
    for subject in subjects:
        for target in targets:
            print(
                f"Running subject_spec_transfer_{TRANSFER_MODE}_bcos | "
            ...
            # Create model name
            model_name = f"subject_spec_transfer_{TRANSFER_MODE}_{subject_str}_{target}_bcos"
            ...
            pretrained_weights = Path("results/models") / f"gen_loto_bcos_{target}.pth" # B-cos model
            if not pretrained_weights.exists():
                raise FileNotFoundError(f"Missing {pretrained_weights}")

            # Metadata for CSV
            row_info = {
                "experiment_type": "subject_spec_transfer_bcos",
    ...
    print(f"Finished subject_spec_transfer_{TRANSFER_MODE}_bcos")
