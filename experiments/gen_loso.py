# experiments/gen_loso.py

from pathlib import Path

import pandas as pd

from data_utils.dataset import load_metadata
from data_utils.splits import make_general_loso_split
from experiments.common import train_and_evaluate
from experiments.config import get_base_settings


def main():

    # Settings for this experiment
    SETTINGS = {
        **get_base_settings(),
        "results_dir": "results/generalized",
        "results_file": "gen_loso.csv",
        "history_file": "gen_loso_history.csv",
    }

    # Get all subjects from general dataset
    subjects = sorted(load_metadata(SETTINGS["csv_path"])["subject"].unique())

    all_results = []
    all_history = []

    # Loop over subjects (Leave-One-Subject-Out)
    for subject in subjects:
        print(f"Running gen_loso | held_out_subject={subject}")

        subject_str = str(subject).zfill(3)

        # Create model name
        model_name = f"gen_loso_{subject_str}"

        # Create train/test split
        train_df, test_df = make_general_loso_split(
            test_subject=subject_str,
            csv_path=SETTINGS["csv_path"],
        )

        # Metadata for CSV
        row_info = {
            "experiment_type": "general_loso",
            "subject_spec_id": None,
            "held_out_subject": subject_str,
            "held_out_target": None,
            "init_mode": "scratch",
        }

        # Run training pipeline
        result_row, history_rows = train_and_evaluate(
            settings=SETTINGS,
            model_name=model_name,
            train_df=train_df,
            test_df=test_df,
            row_info=row_info,
            pretrained_weights=None,
        )

        all_results.append(result_row)
        all_history.extend(history_rows)

    # Save results
    results_dir = Path(SETTINGS["results_dir"])
    results_dir.mkdir(parents=True, exist_ok=True)

    pd.DataFrame(all_results).to_csv(results_dir / SETTINGS["results_file"], index=False)
    pd.DataFrame(all_history).to_csv(results_dir / SETTINGS["history_file"], index=False)

    print("Finished gen_loso")


if __name__ == "__main__":
    main()